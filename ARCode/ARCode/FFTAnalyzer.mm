...
const UInt32 accumulatorDataLenght = 44100/21;//131072;  //16384; //32768; 65536; 131072;
...
static BOOL accumulateFrames(Float32 *frames, UInt32 lenght) { //returned YES if full, NO otherwise.
    ...
    if (accumulatorFillIndex>=accumulatorDataLenght) { return YES; } else {
        UInt32 len = MIN(lenght, accumulatorDataLenght- accumulatorFillIndex);
        memmove(dataAccumulator+accumulatorFillIndex, frames, sizeof(Float32)*len);
        accumulatorFillIndex = accumulatorFillIndex+len;
...
__weak id<FFTAnalyzerDelegate> delegate = nil;
